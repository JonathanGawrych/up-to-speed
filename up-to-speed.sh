...
# Atom packes
if [ "$editor" == "A" ] || [ "$editor" == "B" ] || [ "$editor" == "a" ] || [ "$editor" == "b" ]
	then apm install color-picker linter linter-jshint css-color-highlight emmet editorconfig file-icons minimap minimap-git-diff minimap-color-highlight image-view git-difftool autocomplete-plus autocomplete-snippets autocomplete-paths
fi
